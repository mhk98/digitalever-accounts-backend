...
const getAllFromDB = async (filters, options) => {
  ...
  //   andConditions.push({
  //     [Op.or]: ProfileLossSearchableFields.map((field) => ({
  ...
  const result = await ProfileLoss.findAll({
  ...
  const total = await ProfileLoss.count({ where: whereConditions });
...
const getDataById = async (id) => {
  const result = await ProfileLoss.findOne({
    where: {
      Id: id,
    },
  });
...
const deleteIdFromDB = async (id) => {
  const result = await ProfileLoss.destroy({
    where: {
      Id: id,
    },
  });
...
//   const [updatedCount] = await ProfileLoss.update(data, {
//     where: { Id: id },
//   });
...
const updateOneFromDB = async (id, payload) => {
  const [updatedCount] = await ProfileLoss.update(payload, {
    where: { Id: id },
  });
...
module.exports = ProfileLossService;
